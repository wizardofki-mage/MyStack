...
- So, an ArrayList is a more efficient data structure to use in creating a stack than a LinkedList since interaction with the stack only happens at the top of that stack, and these operations are O(1) with a backing ArrayList vs O(n) with a singly-linked LinkedList.
